...
- After each integrated iteration, report an approximate completion range against the entire original Markdown, the actual capabilities added, the verification level, and the largest remaining gaps. Report first-stage completion separately; never substitute it or the test pass rate for whole-product completion.
