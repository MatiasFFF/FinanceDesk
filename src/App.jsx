...
const WorkspaceManager = lazy(() => import("./features/workspaces/WorkspaceManager.jsx").then((module) => ({ default: module.WorkspaceManager })));
const MemberLedgerPage = lazy(() => import("./features/members/MemberLedgerPage.jsx").then((module) => ({ default: module.MemberLedgerPage })));
const InventoryPage = lazy(() => import("./features/inventory/InventoryPage.jsx").then((module) => ({ default: module.InventoryPage })));
const ManualVoucherPanel = lazy(() => import("./features/accounting/ManualVoucherPanel.jsx").then((module) => ({ default: module.ManualVoucherPanel })));
const AccountingWorkbench = lazy(() => import("./features/accounting/AccountingWorkbench.jsx").then((module) => ({ default: module.AccountingWorkbench })));
const ReceivablesPayablesPanel = lazy(() => import("./features/accounting/AccountingWorkbench.jsx").then((module) => ({ default: module.ReceivablesPayablesPanel })));
const DocumentIntakePanel = lazy(() => import("./features/intake/DocumentIntakePanel.jsx").then((module) => ({ default: module.DocumentIntakePanel })));

function DeferredView({ active, children, label, fullPage = false }) {
  const [hasOpened, setHasOpened] = useState(active);
  useEffect(() => { if (active) setHasOpened(true); }, [active]);
  if (!active && !hasOpened) return null;
  return (
    <div hidden={!active}>
      <Suspense fallback={<div className={fullPage ? "page-content" : undefined}><p className="quiet-copy" role="status" style={{ margin: 0, fontSize: "var(--font-body, 14px)" }}>正在加载{label}…</p></div>}>
        {children}
      </Suspense>
    </div>
  );
...
const PAYROLL_WORKPAPER_ROW_IDS = new Set([
  "payroll",
...
function requiredInitialConfirmationSections(workspace) {
  return [
    "finance",
    "revenue",
    "costExpense",
    "vat",
    "inputVat",
    ...(workspaceModuleEnabled(workspace, "payroll") ? ["payroll", "socialSecurity"] : []),
    "openItems",
  ];
}

function enabledWorkflowChecks(workspace, checks) {
  if (workspaceModuleEnabled(workspace, "payroll")) return checks;
...
function pageHeading(page, workspace) {
...
function navigationItemLabel(item, workspace, short = false) {
  if (item.id !== "members") return short ? item.shortLabel : item.label;
  const { member } = workspaceTerminology(workspace);
  return short ? member : `${member}台账`;
}
        ...
        {navigation.map((item) => {
          ...
          return (
            <button className={page === item.id ? "active" : ""} key={item.id} onClick={() => onPage(item.id)} type="button">
              ...
              <span>{navigationItemLabel(item, workspace)}</span>
            </button>
          );
        })}
      </nav>
  ...
  return (
    <nav className="bottom-nav" aria-label="移动端导航">
      {primaryNavigationForWorkspace(workspace).map((item) => {
        const Icon = PAGE_ICONS[item.id];
        return <button className={page === item.id ? "active" : ""} key={item.id} onClick={() => onPage(item.id)} type="button"><Icon size={19} weight={page === item.id ? "fill" : "regular"} /><span>{navigationItemLabel(item, workspace, true)}</span></button>;
      })}
    </nav>
  );
}
...
function Topbar({ state, workspace, page, workspaceOverlayOpen, onImport, onSwitchWorkspace, onOpenWorkspaceDialog }) {
  ...
  const [title, subtitle] = pageHeading(page, workspace);
  useEffect(() => {
...
function OverviewPage({ workspace, onPage, onResolveNotice }) {
  ...
  const payrollConfirmationDone = !payrollEnabled || ["payroll", "socialSecurity"].every((id) => flow.checks.find((item) => item.id === id)?.ok);
  const financeConfirmationDone = Boolean(flow.checks.find((item) => item.id === "finance")?.ok && payrollConfirmationDone);
  const exceptionCheck = flow.checks.find((item) => item.id === "exceptions");
  ...
  const nextAction = openNotices.length
    ...
    : !flow.version
      ...
      : taxEnabled && !financeConfirmationDone
        ? { title: `请${terminology.customer}完成首次数据确认`, body: payrollEnabled ? "财务数据与工资社保会分别留下确认记录。" : "财务、税务与报表会留下逐项确认记录。", page: "tax", action: "开始确认" }
        : taxEnabled && !flow.checks.find((item) => item.id === "owner")?.ok
          ...
          : taxEnabled && !workspace.delivery.filing.receipt
            ...
            : !workspace.delivery.filing.archivedAt
              ? { title: taxEnabled ? "回执齐全，可以完成归档" : "可以完成本期归档", body: "归档会保留本期交付记录，并准备下一期期初。", page: "archive", action: "进入归档" }
  ...
  return (
    <div className="page-content overview-page">
      ...
      <section className="metric-grid four">
        ...
        {taxEnabled && <MetricCard label="预计税费" value={formatCurrency(snapshot.summary.estimatedTax)} note="本地估算，不可直接申报" icon={Receipt} tone="clay" onClick={() => onPage("tax")} />}
      </section>
      <BoundaryNote />
    </div>
  );
...
function TransactionDetail({ workspace, transaction, onClose, onStatus, onSaveReview, onEvidence, onLinkEvidence, onDownloadEvidence, onUnlinkEvidence, onToast }) {
  ...
  useEffect(() => {
    if (!payrollEnabled && ["工资表", "社保数据"].includes(evidenceCategory)) setEvidenceCategory("发票");
...
function DrilldownPanel({ row, sectionLabel, terminology, onClose }) {
  useEffect(() => {
  ...
  if (!row) return null;
  const localizedSectionLabel = businessTermCopy(sectionLabel, terminology);
  const localizedRowLabel = businessTermCopy(row.label, terminology);
  return <div className="report-detail-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside aria-label={`${localizedSectionLabel} · ${localizedRowLabel}数字追溯`} aria-modal="true" className="detail-panel report-detail" role="dialog"><div className="detail-heading"><div><h2>{localizedRowLabel}</h2><p>{localizedSectionLabel} · 数字追溯</p></div><button className="icon-button compact" onClick={onClose} type="button" aria-label="关闭下钻"><X size={19} /></button></div><div className="detail-scroll"><div className="drill-total"><span>报表金额</span><strong>{formatCurrency(row.value)}</strong></div>{row.formula && <div className="drill-formula"><small>计算口径</small><strong>{businessTermCopy(row.formula, terminology)}</strong></div>}{row.details?.length ? <div className="drill-list">{row.details.map((item) => <div key={item.id}><span><strong>{businessTermCopy(item.title, terminology)}</strong><small>{item.date || "—"} · {businessTermCopy(item.reference || "本地记录", terminology)}</small>{item.description && <small>{businessTermCopy(item.description, terminology)}</small>}{(item.sourceIds?.length || item.evidenceIds?.length) && <small>{item.sourceIds?.length || 0} 个业务来源 · {item.evidenceIds?.length || 0} 份凭证附件</small>}</span><b>{formatCurrency(item.amount, { sign: true })}</b></div>)}</div> : <EmptyState title="当前数字来自明确计算口径" description={businessTermCopy(row.formula || "没有额外的单笔来源。", terminology)} />}</div></aside></div>;
}

function storeReportMetrics(terminology) {
  return [
    { id: "collections", label: "本期收款" },
    { id: "recognizedRevenue", label: "确认收入" },
    { id: "refunds", label: "退款" },
    { id: "coachCommission", label: `${terminology.coach}提成` },
    { id: "grossProfit", label: "毛利" },
    { id: "unfulfilledBalance", label: "预收 / 未履约" },
  ];
}

function StoreManagementReport({ report, terminology }) {
  const metrics = storeReportMetrics(terminology);
  return (
    <section className="panel store-management-report">
      <div className="panel-heading"><div><h2>{terminology.location}经营</h2><p>仅统计已入账业务；毛利 = 确认收入 − {terminology.coach}提成。展开可追溯{terminology.member}、业务与凭证。</p></div><span>{report.stores.length} 家{terminology.location}</span></div>
      {report.postingCoverage.unpostedEventCount > 0 && <div className="store-report-notice"><WarningCircle size={16} /><span>本期另有 {report.postingCoverage.unpostedEventCount} 笔已确认业务尚未入账，暂不进入{terminology.location}报表。</span></div>}
      <div className="store-report-table">
        <div className="store-report-row heading"><span>{terminology.location}</span>{metrics.map((metric) => <span key={metric.id}>{metric.label}</span>)}</div>
        {report.stores.map((store) => (
          <details className="store-report-store" key={store.id}>
            <summary className="store-report-row"><span><strong>{store.name}</strong><small>{store.sources.length} 条来源 · {store.voucherIds.length} 张凭证</small></span>{metrics.map((metric) => <strong key={metric.id}>{formatCurrency(store.metrics[metric.id])}</strong>)}</summary>
            <div className="store-report-drilldown">
              <section>
                <div className="subheading"><strong>{terminology.member}明细</strong><span>{store.members.length} 名</span></div>
                {store.members.length ? <div className="store-member-list">{store.members.map((member) => <article key={member.id}><span><strong>{member.name}</strong><small>{[member.coach, member.department, member.project].filter(Boolean).map((value) => businessTermCopy(value, terminology)).join(" · ") || `未记录${terminology.coach} / 部门 / 项目`}</small></span><span><small>收款 / 收入</small><strong>{formatCurrency(member.metrics.collections)} / {formatCurrency(member.metrics.recognizedRevenue)}</strong></span><span><small>退款 / 提成</small><strong>{formatCurrency(member.metrics.refunds)} / {formatCurrency(member.metrics.coachCommission)}</strong></span><span><small>未履约</small><strong>{formatCurrency(member.metrics.unfulfilledBalance)}</strong></span></article>)}</div> : <p className="quiet-copy">当前{terminology.location}没有{terminology.member}维度来源。</p>}
              </section>
              <section>
                ...
                {store.sources.length ? <div className="store-source-list">{store.sources.map((source) => {
                  const impacts = metrics.filter((metric) => Number(source.impacts?.[metric.id] || 0) !== 0);
                  return <article key={source.id}><span><strong>{businessTermCopy(source.label, terminology)}{source.memberName ? " · " + source.memberName : ""}</strong><small>{source.date || "期初"} · {[source.coach, source.department, source.project].filter(Boolean).map((value) => businessTermCopy(value, terminology)).join(" · ") || `未记录${terminology.coach} / 部门 / 项目`}</small><small>{source.voucherIds.length ? "凭证 " + source.voucherIds.join("、") : `${terminology.member}期初来源`} · 事件 {source.eventId || source.id}</small></span><span>{impacts.map((metric) => <small key={metric.id}>{metric.label} {formatCurrency(source.impacts[metric.id], { sign: true })}</small>)}</span></article>;
                })}</div> : <p className="quiet-copy">尚无已入账{terminology.member}业务来源。</p>}
              </section>
            </div>
          </details>
        ))}
...
function GenericStoreManagementReport({ report }) {
  ...
  return (
    <section className="panel store-management-report generic-store-management-report">
      <div className="panel-heading"><div><h2>场所经营</h2><p>汇总本期已入账收入、成本、费用与利润；未归属场所的数据单列，展开可追溯来源。</p></div><span>{stores.length ? `${stores.length} 个场所` : "等待经营数据"}</span></div>
...
function BillSourceRows({ rows, terminology }) {
  return rows.length ? <div className="owner-source-rows">{rows.map((row) => {
    ...
    return <article key={row.billId || row.id}><span><strong>{row.counterparty || businessTermCopy(row.label || row.billNo, terminology)}</strong><small>{businessTermCopy(row.summary || row.reference || "本地业务来源", terminology)}</small><small>{businessTermCopy(row.billNo || row.reference || row.billId, terminology)} · {timing} · 来源 {row.sourceIds?.join("、") || "—"}</small></span><strong>{formatCurrency(row.balance ?? row.availableBalance ?? row.amount)}</strong></article>;
  })}</div> : <p className="quiet-copy">当前没有对应来源。</p>;
}

function ForecastEventRows({ events, terminology }) {
  return events.length ? <div className="owner-source-rows">{events.map((event) => <article key={event.id}><span><strong>{businessTermCopy(event.label, terminology)}</strong><small>{event.dueDate ? "原到期日 " + event.dueDate + (event.overdueAtStart ? " · 已逾期，列入预测首日" : "") : "缺少明确支付 / 回款日期"}{event.basis ? " · " + businessTermCopy(event.basis, terminology) : ""}</small><small>{businessTermCopy(event.reference || "本地记录", terminology)} · 来源 {event.sourceIds?.join("、") || "—"}</small></span><strong>{formatCurrency(event.cashEffect ?? (event.type === "receivable" ? event.amount : -event.amount), { sign: true })}</strong></article>)}</div> : <p className="quiet-copy">当前没有对应来源。</p>;
}

function OwnerLiquidityReport({ management, terminology, payrollEnabled }) {
  ...
  const forecastTotals = [
    ...
    ...(payrollEnabled ? [
      { id: "payroll", label: `已确认${terminology.personnel}工资` },
      { id: "socialSecurity", label: `已确认${terminology.personnel}社保` },
    ] : []),
    { id: "tax", label: "已确认税款" },
  ];
  return (
    <section className="panel owner-liquidity-report">
      <div className="panel-heading"><div><h2>账龄与未来 30 天现金</h2><p>账龄只统计未核销的普通应收、应付；预收与预付单列。现金预测仅采用明确到期日和已确认金额，缺日期项目不进入曲线。</p></div><span>截至 {ageing.asOf}</span></div>
      <div className="owner-liquidity-layout">
        <section className="owner-report-section">
          ...
          <div className="ageing-table">
            ...
            {ageing.buckets.map((bucket) => <details key={bucket.id}><summary className="ageing-row"><span>{bucket.label}<small>{bucket.rows.length} 笔</small></span><strong>{formatCurrency(bucket.receivable.value)}</strong><strong>{formatCurrency(bucket.payable.value)}</strong></summary><BillSourceRows rows={bucket.rows} terminology={terminology} /></details>)}
          </div>
          <details className={ageing.missingDueDate.rows.length ? "owner-missing-sources warning" : "owner-missing-sources"} open={ageing.missingDueDate.rows.length > 0}>
            ...
            <BillSourceRows rows={ageing.missingDueDate.rows} terminology={terminology} />
          </details>
          <div className="advance-exclusion">
            <div><strong>预收 / 预付单独管理</strong><small>{businessTermCopy(ageing.excludedAdvances.reason, terminology)}</small></div>
            <details><summary>{terminology.customer}预收 <strong>{formatCurrency(ageing.excludedAdvances.customerDeposits.value)}</strong></summary><BillSourceRows rows={ageing.excludedAdvances.customerDeposits.rows} terminology={terminology} /></details>
            <details><summary>{terminology.supplier}预付 <strong>{formatCurrency(ageing.excludedAdvances.supplierPrepayments.value)}</strong></summary><BillSourceRows rows={ageing.excludedAdvances.supplierPrepayments.rows} terminology={terminology} /></details>
          </div>
        </section>

        <section className="owner-report-section">
          <div className="subheading"><strong>未来 30 天现金预测</strong><span>{forecast.startDate} 至 {forecast.endDate}</span></div>
          <p className="forecast-formula">{businessTermCopy(forecast.formula, terminology)}</p>
          <div className="forecast-key-grid">
            <details><summary><span>当前银行余额</span><strong>{formatCurrency(forecast.currentBalance.value)}</strong></summary><BillSourceRows rows={forecast.currentBalance.accounts.map((account) => ({ ...account, kind: "account", amount: account.value, reference: account.id }))} terminology={terminology} /></details>
            <details><summary><span>最低预测余额</span><strong>{formatCurrency(forecast.minimumBalance.value)}</strong><small>{forecast.minimumBalance.date}</small></summary><BillSourceRows rows={forecast.currentBalance.accounts.map((account) => ({ ...account, kind: "account", amount: account.value, reference: account.id }))} terminology={terminology} /><ForecastEventRows events={forecast.minimumBalance.events} terminology={terminology} /></details>
            <details className={forecast.cashShortfall.value > 0 ? "danger" : ""}><summary><span>现金缺口</span><strong>{formatCurrency(forecast.cashShortfall.value)}</strong><small>{forecast.deficitDate || "预测期内无缺口"}</small></summary><BillSourceRows rows={forecast.currentBalance.accounts.map((account) => ({ ...account, kind: "account", amount: account.value, reference: account.id }))} terminology={terminology} /><ForecastEventRows events={forecast.cashShortfall.events} terminology={terminology} /></details>
          </div>
          <div className="forecast-total-grid">{forecastTotals.map((item) => {
            const total = forecast.totals[item.id];
            return <details key={item.id}><summary><span>{item.label}</span><strong>{formatCurrency(total.value)}</strong></summary><ForecastEventRows events={total.events} terminology={terminology} /></details>;
          })}</div>
...
function ReportsPage({ workspace, onPage, onFreeze, onExportExcel }) {
  ...
  useEffect(() => {
    if (latestVersionId && latestVersionId !== knownLatestVersion.current) setVersionId(latestVersionId);
    knownLatestVersion.current = latestVersionId;
  }, [latestVersionId]);
  ...
  const statementChecks = Object.entries(live.summary.engineChecks || {})
    ...
    .map(([id, check]) => {
      const copy = REPORT_RECONCILIATION_COPY[id] || { label: check.label || id, formula: check.formula || "" };
      if (id === "memberService") {
        return { id, ...check, ...copy, label: `${terminology.service}履约勾稽`, formula: `${terminology.service}台账未履约余额 = 合同负债余额` };
      }
      return { id, ...check, ...copy, label: businessTermCopy(copy.label, terminology), formula: businessTermCopy(copy.formula, terminology) };
    });
  ...
  return (
    <div className="page-content reports-page">
      ...
      <div className="reports-layout">
        ...
        <aside className="panel version-panel"><div className="panel-heading"><div><h2 className="card-title"><Clock size={18} /><span>报表版本与差异</span></h2></div></div>{versions.length ? <div className="version-list">{versions.map((version, index) => <button className={version.id === versionId ? "active" : ""} key={version.id} onClick={() => setVersionId(version.id)} type="button"><span><strong>{version.label}</strong><small>{formatDateTime(version.createdAt)} · {version.actor}</small></span><TonePill tone="success">{index === 0 ? "最新冻结" : "已冻结"}</TonePill></button>)}</div> : <EmptyState title="还没有冻结版本" description="勾稽通过后可冻结首个版本，每个冻结版本独立保留。" />}<div className="version-diff"><div className="subheading"><strong>{previous ? `${latest.label} 对比 ${previous.label}` : "版本差异"}</strong><span>{differences.length} 项变化</span></div>{previous ? (differences.length ? differences.slice(0, 8).map((item) => <div key={item.id}><span><small>{businessTermCopy(item.section, terminology)}</small><strong>{businessTermCopy(item.label, terminology)}</strong></span><b className={item.delta > 0 ? "income" : "expense"}>{formatCurrency(item.delta, { sign: true })}</b></div>) : <p className="quiet-copy">最新两个版本的报表数字一致，时间与确认记录仍分别保留。</p>) : <p className="quiet-copy">冻结第二个版本后，这里会逐项显示与上一版本的差异。</p>}</div><div className="report-export-history"><div className="subheading"><strong>Excel 本地导出</strong><span>{reportExports.length} 次</span></div>{reportExports.length ? reportExports.slice(0, 3).map((item) => <article key={item.id}><DownloadSimple size={17} /><span><strong>{item.reportVersionLabel} · {item.fileName}</strong><small>{formatDateTime(item.exportedAt)} · {fileSize(item.size)} · 仅本地，未上传</small></span></article>) : <p className="quiet-copy">当前期间还没有 Excel 导出记录。</p>}</div>{taxEnabled && <button className="secondary-button wide" onClick={() => onPage("tax")} type="button">进入确认与申报<ArrowRight size={16} /></button>}</aside>
      </div>
      {sectionId === "owner" && !managementReportAllHidden && <>{memberBusinessEnabled ? <StoreManagementReport report={storeReport} terminology={terminology} /> : <GenericStoreManagementReport report={storeReport} />}<OwnerLiquidityReport management={management} terminology={terminology} payrollEnabled={payrollEnabled} /></>}
      <BoundaryNote />
...
function TaxPage({ workspace, onPage, onTaxChange, onTaxCommit, onSectionDecision, onPrepareDraft, onFinalConfirm, onExport, onReceipt, onToast }) {
  ...
  const activeConfirmation = [...(workspace.confirmations || [])].reverse().find((confirmation) => (
    customerConfirmationMatchesVersion(confirmation, version)
  ));
  ...
  const summarizeSources = (rows, fallback) => {
    ...
    if (details.length) {
      ...
      return businessTermCopy(`${details.length} 条本地来源${labels.length ? ` · ${labels.join("、")}` : ""}`, terminology);
    }
    const formula = rows.find((row) => row?.formula)?.formula;
    return businessTermCopy(formula ? `计算口径：${formula}` : fallback, terminology);
  };
  const confirmationSections = buildCustomerConfirmationSections(snapshot, { payrollEnabled });
  const visibleWorkpaperRows = payrollEnabled ? snapshot.taxWorkpaper.rows : snapshot.taxWorkpaper.rows.filter((row) => !PAYROLL_WORKPAPER_ROW_IDS.has(row.id));
  const pendingSources = confirmationSections.openItems.items;
  const confirmationItems = [
    { id: "revenue", label: "收入", amount: formatCurrency(confirmationSections.revenue.value), sourceSummary: summarizeSources(confirmationSections.revenue.rows, "来自当前冻结利润表"), majorByDefault: true },
    { id: "costExpense", label: "成本费用", amount: formatCurrency(confirmationSections.costExpense.value), sourceSummary: summarizeSources(confirmationSections.costExpense.rows, "来自当前冻结利润表及已入账凭证"), majorByDefault: true },
  ...
  const nextStep = blockers.length
    ...
    : !exportReady ? { title: "处理申报包剩余条件", label: "查看差异与条件", run: () => jumpTo("tax-workpaper") }
    : !filing.exportedAt ? { title: "两次确认已完成", label: "查看申报包", run: () => jumpTo("tax-package") }
    : !filing.receipt ? { title: "等待真实外部办理回执", label: "导入回执", run: () => jumpTo("tax-package") }
    : { title: "资料已具备归档条件", label: "前往资料归档", run: () => onPage("archive") };
                ...
                <article className={`confirmation-item ${status} ${expanded ? "expanded" : "collapsed"}`} key={item.id}>
                  ...
                  {locked ? (
                    ...
                    <div className="confirmation-form">
                      ...
                      {decision === "reject" && <p className="confirmation-reject-warning"><WarningCircle size={15} />保存后将立即生成待复核事项并返回异常处理。</p>}
                      <button className={decision === "reject" ? "danger-button" : "secondary-button"} disabled={!canSave} onClick={() => {
                        const saved = onSectionDecision({ reportVersionId: version.id, section: item.id, decision, note: note.trim(), isMajor, responsibleName: responsibleName.trim(), confirmationName: initialConfirmer.trim() });
                        if (saved) setSectionDrafts((current) => ({ ...current, [item.id]: {} }));
                      }} type="button">{decision === "reject" ? "保存异议并返回复核" : "保存本项确认"}</button>
                    </div>
                  )}</div>}
                </article></div>
              );
      ...
      <section className="panel final-confirm-panel final-confirm-workspace" id="tax-final-confirmation" hidden={!filing.draftCreatedAt && !storedFinalConfirmation}>
        ...
        {finalConfirmationCurrent ? (
          ...
          <div className="final-confirm-form">
            ...
            <button className="primary-button" disabled={!finalReady} onClick={() => onFinalConfirm({ reportVersionId: version.id, filingDraftCreatedAt: filing.draftCreatedAt, name: confirmer.trim(), selections: { ...finalChecks, deductionAuthorization } })} type="button"><ShieldCheck size={17} />保存最终确认（不提交税务局）</button>
          </div>
        )}
      </section>
    </div>
...
function ArchivePage({ workspace, onPage, onDocuments, onDownloadDocument, onReceipt, onArchive, onNextPeriod, onExportIndex, onExportArchive }) {
  ...
  const equityAccounts = workspaceAccountDefinitions(workspace).filter((account) => account.category === "equity" && account.status !== "inactive");
  const selectedEquityAccountId = carryForwardAccountId
  ...
  function showArchiveTab(nextTab) {
    setTab(nextTab);
    if (nextTab !== "documents") setQuery("");
  }
  function openReceiptStatus() {
    if (!filing.receipt) {
      if (filing.exportedAt) receiptInput.current?.click();
      else onPage("tax");
      return;
    }
    setTab("documents");
    setQuery(filing.receipt.name || "");
  }
  return (
    <div className="page-content archive-page">
      <StageRail workspace={workspace} onPage={onPage} />
      <section className="archive-hero"><div><h2>{archived ? `${formatPeriod(workspace.currentPeriod)} 已归档` : "本期资料与归档"}</h2><p>{archived ? `归档于 ${formatDateTime(archived.archivedAt)}。已保存本期报表与资料记录，原件状态见文件列表。` : taxEnabled ? "必须先导入真实外部办理回执，再把本地申报包、确认记录和操作日志一起归档。" : "把当前冻结报表、本期资料和操作记录建立本地归档索引。"}</p></div><div className="archive-hero-actions">{archived ? <>
      ...
      <section className="panel archive-content-panel">
        ...
        {tab === "periods" && <div className="archive-history-tools"><button className="secondary-button" onClick={onExportIndex} type="button"><DownloadSimple size={17} />导出归档索引</button></div>}
...
function WorkspaceDialog({ mode, workspace, onClose, onSubmit }) {
  const terminology = mode === "create" ? DEFAULT_TERMINOLOGY : workspaceTerminology(workspace);
    ...
    <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <form className="modal-card workspace-dialog" role="dialog" aria-modal="true" aria-labelledby="workspace-dialog-title" onSubmit={submit}>
        <div className="modal-heading">
          <div><h2 id="workspace-dialog-title">{title}</h2></div>
...
function App() {
  ...
  function addLedgerMember(values) {
    try {
      mutateActive((current) => audit(
        ...
        `新增${terminology.member}`,
        `${values.name} · ${values.coach || `未分配${terminology.coach}`}`,
        actorName,
      ));
      setToast({ tone: "success", message: `${terminology.member}“${values.name}”已保存到当前工作台` });
      return true;
    } catch (error) {
      setToast({ tone: "danger", message: memberRoleCopy(error.message || `${terminology.member}保存失败`, terminology) });
      return false;
    }
  }
  function changeLedgerMemberStatus(memberId, status) {
    try {
      ...
      mutateActive((current) => audit(
        ...
        `更新${terminology.member}状态`,
        `${member?.name || memberId} → ${label}`,
        actorName,
      ));
      setToast({ tone: "success", message: `${member?.name || terminology.member}已更新为“${label}”` });
    } catch (error) {
      setToast({ tone: "danger", message: memberRoleCopy(error.message || `${terminology.member}状态更新失败`, terminology) });
    }
  }
  function addLedgerEvent(values) {
    try {
      ...
      mutateActive((current) => audit(
        ...
        `新增${memberRoleCopy(definition?.label || `${terminology.member}业务`, terminology)}`,
      ...
      setToast({ tone: "danger", message: memberRoleCopy(error.message || `${terminology.member}业务保存失败`, terminology) });
      return false;
    }
  }
  function changeLedgerEventStatus(eventId, status) {
    try {
      ...
      mutateActive((current) => audit(
        ...
        `更新${terminology.member}业务状态`,
        `${event?.accountingLabel || event?.memberName || eventId} → ${status}`,
        actorName,
      ));
      setToast({ tone: "success", message: status === "void" ? "业务已作废，余额已同步恢复" : `业务状态与${terminology.member}余额已更新` });
    } catch (error) {
  ...
  function commitTax(field) {
    try {
      const labels = { vatRate: "增值税率", surtaxRate: "附加税率", incomeTaxRate: "所得税率", adjustments: "增值税计税基础调整", payroll: "工资薪金", socialSecurity: "社保数据", note: "复核备注" };
  ...
  function recordInitialConfirmationSection(input) {
    try {
      ...
      if (input.decision === "reject") {
        ...
        setToast({ tone: "success", message: "本项确认已保存到本地记录" });
      }
      return true;
    } catch (error) {
      setToast({ tone: "danger", message: error.message || `${terminology.customer}确认记录失败` });
      return false;
    }
  }
  ...
  function prepareDraft() {
    try {
      mutateActive((current) => {
        const flow = workflowChecks(current);
        const requiredSections = requiredInitialConfirmationSections(current);
        const prepareChecks = enabledWorkflowChecks(current, flow.prepare);
        ...
        const confirmationComplete = Boolean(
          customerConfirmationMatchesVersion(confirmation, flow.version)
          && requiredSections.every((section) => confirmation.sections?.[section]?.status === "approved"),
        );
        if (!confirmationComplete) throw new Error(`还不能生成底稿：${requiredSections.length} 项${terminology.customer}确认尚未全部完成`);
        const missing = prepareChecks.filter((item) => !item.ok);
  ...
  function completeArchive() {
    try {
      const missing = enabledWorkflowChecks(workspace, workflowChecks(workspace).archive).filter((item) => !item.ok);
  ...
  return (
    <div className="app-shell">
      ...
      <div className="app-main">
        ...
        {workspaceModuleEnabled(workspace, "members") && <DeferredView key={`${workspace.id}-members`} active={activePage === "members"} label="会员台账" fullPage><MemberLedgerPage workspace={workspace} onAddMember={addLedgerMember} onMemberStatus={changeLedgerMemberStatus} onAddEvent={addLedgerEvent} onEventStatus={changeLedgerEventStatus} /></DeferredView>}
      ...
      {managerHasOpened && <Suspense fallback={managerOpen ? (
        <div className="modal-backdrop foundation-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setManagerOpen(false)} onKeyDown={(event) => { if (event.key === "Escape") { event.stopPropagation(); setManagerOpen(false); } }}>
          <section className="modal-card foundation-manager" role="dialog" aria-modal="true" aria-label="工作台管理">
            <div className="modal-heading"><h2>工作台管理</h2><button className="icon-button compact" type="button" autoFocus aria-label="关闭" onClick={() => setManagerOpen(false)}><X size={19} /></button></div>
            <p className="quiet-copy" role="status">正在加载工作台管理…</p>
          </section>
        </div>
      ) : null}>
