...
export function usePeriodLeaveGuard({ dirty = false, busy = false } = {}) {
  ...
  useEffect(() => {
    function beforeChange(event) {
      ...
      if (dirty && event.detail.ignoreDirtyGuardId !== guardId) event.detail.dirty = true;
    }
...
export function allowPeriodNavigation({ ignoreDirtyGuardId } = {}) {
  const detail = { busy: false, dirty: false, ignoreDirtyGuardId };
