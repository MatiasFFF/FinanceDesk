...
export const AI_FINANCE_TOOLS = [
  ...
  tool("read_bank_statement", "确定性读取已保存银行原件的全部流水，返回每个真实账户分组及程序计算的全量行数、收支、日期范围和错误。无需先创建账户；样例不是全文件，不能用样例推算总额。", {
    documentId: text,
  }, ["documentId"]),
  tool("prepare_bank_import", "读取银行原件并按真实账户逐组准备用户确认事项，不直接创建账户或导入。只需documentId；程序识别列、日期、账户并计算全部收支。已有账户唯一匹配时复用，缺少账户提出源文件建议，歧义由用户在确认卡片选择。指定已有账户时只能用于sourceGroupId对应的单组，绝不能混合其他组。", {
    documentId: text, sourceGroupId: text, accountId: text, mapping: { type: "object", additionalProperties: { type: "integer", minimum: 0 } },
  }, ["documentId"]),
...
明确区分“原件已保存”“字段/导入/业务待确认”“业务已确认但凭证未入账”和“凭证已入账”。只按本轮工具结果或最新上下文描述完成状态；收到pending_confirmation表示已经生成待确认建议，不要重复生成，也不能说已导入或已入账。already_imported表示同一原件本组流水已存在、本次没有新增，不需要再次确认；不能说本轮新导入了这些金额。superseded是已被替换或无需再次确认的旧建议，不能继续使用。
银行分组和全量收支以程序返回的analysis及group.summary为准，不能用前20行或其他样例估计全文件，不能自行凑总额。原件已保存后即使工作台没有账户也能读取和准备，绝不能要求先手动建立账户或拆表。多账户文件逐组确认，缺少账户可随对应组一起确认创建，不能合并导入。不要向用户询问技术列号、mapping或日期格式；程序无法识别时说明具体缺失的业务字段，保留原件与全部未处理行。
工具返回needs_input、needs_mapping或needs_correction时，阅读具体缺项；只询问无法从原件唯一确定的业务事实。不要反复提交相同错误参数，也不要为了通过校验编造编号、账户、交易对手、税务或发票状态。取消、权限、目标或原件异常必须停止。任何继续或重试都不能自动重放用户确认。
